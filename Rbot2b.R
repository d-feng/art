...
## random fold change
x <- rnorm(148, mean=0, sd=2)
mt=cbind(tsne_out$Y,x)
#result=kde2d(tsne_out$Y[,1],tsne_out$Y[,2],h=5)
result=kde2d(mt[,1],mt[,2],n=100,h=5)
#h <-mt[,5]
h=mt[,3]
hcutoff=5
h[is.na(h)] <- 0
h[abs(h)<hcutoff] <- 0
s=interp(mt[,1],mt[,2],log2(h+8),nx=150,ny=150,linear=TRUE,extrap=TRUE,jitter.random = TRUE,jitter = 10^-12, jitter.iter = 6);
